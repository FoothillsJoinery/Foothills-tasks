  ...
  return `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #e8e6df;vertical-align:top">
        <div style="font-size:13px;color:#1a1a18;font-weight:600">${taskTitle}</div>
        <div style="font-size:11px;color:#888780;margin-top:1px;margin-bottom:3px">${pathLine}</div>
        <div style="font-size:13px;color:#1a1a18;margin-top:2px">${need.text}</div>
        <div style="font-size:12px;color:#888780;margin-top:3px">Logged ${fmtDate(need.created_at)} · ${age}</div>
        ${answerLine}
      </td>
    </tr>`
}
  ...
  return `
    <div style="font-family:sans-serif;max-width:600px;color:#1a1a18">
      <div style="margin-bottom:24px">
        ...
        <p style="margin:6px 0 0;font-size:13px;color:#888780">${open.length} open need${open.length !== 1 ? 's' : ''} · ${since ? `logged since ${fmtDate(since)}` : 'all open'} · ${fmtDate(new Date().toISOString())}</p>
      </div>
