  ...
  return `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #e8e6df;vertical-align:top">
        ...
        <div style="font-size:12px;color:#888780;margin-top:3px">${task} · Logged ${fmtDate(need.created_at)} by ${need.requested_by} · ${age}</div>
        ${answerLine}
      </td>
    </tr>`
}

function buildWeeklyHtml(job, needs, taskMap, jobUrl) {
  ...
  for (const cat of CATEGORIES) {
    ...
    categorySections += `
      ...
      <table style="width:100%;border-collapse:collapse">
        ${catNeeds.map(n => needRow(n, taskMap)).join('')}
      </table>`
  }
  ...
  if (uncategorized.length > 0) {
    categorySections += `
      ...
      <table style="width:100%;border-collapse:collapse">
        ${uncategorized.map(n => needRow(n, taskMap)).join('')}
      </table>`
  }
  ...
  return `
    <div style="font-family:sans-serif;max-width:600px;color:#1a1a18">
      <div style="margin-bottom:24px">
        <div style="font-size:11px;font-weight:600;color:#888780;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:4px">Weekly needs report</div>
      ...
      ${resolved.length > 0 ? `
      <div style="margin-bottom:24px">
        <div style="font-size:12px;font-weight:600;color:#1a8a4a;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px">Resolved this period (${resolved.length})</div>
        <table style="width:100%;border-collapse:collapse">
          ${resolved.map(n => needRow(n, taskMap)).join('')}
        </table>
      </div>` : ''}
      ...
      <p style="margin-top:24px;font-size:11px;color:#b4b2a9">Foothills Joinery · This is an automated weekly summary</p>
    </div>`
}
...
export async function sendWeeklyReport(job, jobUrl) {
  ...
  await sendEmail(job.client_email, `Weekly needs report — ${job.name}`, html)
